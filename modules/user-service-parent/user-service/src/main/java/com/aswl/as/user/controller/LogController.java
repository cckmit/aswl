...
import com.aswl.as.user.service.LogService;
...
@Slf4j
@AllArgsConstructor
@Api(value = "/api/v1/log", tags = "日志信息管理")
@RestController
...
public class LogController extends BaseController {
    ...
    public ResponseBean<PageInfo<LogVo>> logList(@RequestParam(value = CommonConstant.PAGE_NUM, required = false, defaultValue = CommonConstant.PAGE_NUM_DEFAULT) String pageNum,
                                                 @RequestParam(value = CommonConstant.PAGE_SIZE, required = false, defaultValue = CommonConstant.PAGE_SIZE_DEFAULT) String pageSize,
                                                 @RequestParam(value = CommonConstant.SORT, required = false, defaultValue = CommonConstant.PAGE_SORT_DEFAULT) String sort,
                                                 @RequestParam(value = CommonConstant.ORDER, required = false, defaultValue = CommonConstant.PAGE_ORDER_DEFAULT) String order,
                                                 Log log) {
        return  new ResponseBean<>(logService.findPageInfo(PageUtil.pageInfo(pageNum,pageSize,sort,order),log));
    }
    ...
    // @PreAuthorize("hasAuthority('system:log:del') or hasAnyRole('" + SecurityConstant.ROLE_ADMIN + "')")
    @ApiOperation(value = "一键删除日志", notes = "一键删除日志")
    public ResponseBean<Boolean> clearAll() {

        boolean success = false;
        try {
               success = logService.clearAll() >= 0;
        } catch (Exception e) {
            log.error("清空日志失败！", e);
        }
        return new ResponseBean<>(success);
