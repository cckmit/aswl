...
@Slf4j
@AllArgsConstructor
@Api(value = "/api/v1/menu", tags = "菜单信息管理")
@RestController
...
public class MenuController extends BaseController {
    ...
    public ResponseBean<List<MenuDto>> userMenu() {
    ...
    @Log(value = "新增菜单",businessType = BusinessType.INSERT)
    ...
    @Log(value = "删除菜单",businessType = BusinessType.DELETE)
    ...
    public ResponseBean<Menu> menu(@PathVariable String id) {
        Menu menu = new Menu();
        menu.setId(id);
        return new ResponseBean<>(menuService.get(menu));
    }
    ...
    public ResponseBean<PageInfo<Menu>> menuList(@RequestParam(value = CommonConstant.PAGE_NUM, required = false, defaultValue = CommonConstant.PAGE_NUM_DEFAULT) String pageNum,
        ...
        return new ResponseBean<>( menuService.findPage(PageUtil.pageInfo(pageNum, pageSize, sort, order), menu));
    }
    ...
    public ResponseBean<List<String>> roleTree(@PathVariable String roleCode) {
        ...
        // 获取菜单ID
        if (Optional.ofNullable(menuStream).isPresent())
            return new ResponseBean<>(menuStream.map(Menu::getId).collect(Collectors.toList()));
        return new ResponseBean<>(new ArrayList<>());
    }
