...
import com.aswl.as.user.service.AppRoleMenuService;
...
import com.aswl.as.user.service.RoleMenuService;
...
@Slf4j
@AllArgsConstructor
@Api(value = "/api/v1/role", tags = "角色信息管理")
@RestController
...
public class RoleController extends BaseController {
    ...
    public ResponseBean<Role> role(@PathVariable String id) {
        Role role = new Role();
        role.setId(id);
        return new ResponseBean<>(roleService.get(role));
    }
    ...
    public ResponseBean<PageInfo<Role>> roleList(@RequestParam(value = CommonConstant.PAGE_NUM, required = false, defaultValue = CommonConstant.PAGE_NUM_DEFAULT) String pageNum,
        ...
        return new ResponseBean<>(roleService.findPage(PageUtil.pageInfo(pageNum, pageSize, sort, order), role));
    }
    ...
    @ApiImplicitParam(name = "role", value = "角色实体role", required = true, dataType = "RoleVo")
    @Log(value = "更新角色菜单",businessType = BusinessType.UPDATE)
     ...
     * @date 2018/10/28 14:20
     */
    @PutMapping("roleAppMenuUpdate")
    @ApiOperation(value = "更新角色APP菜单信息", notes = "更新角色APP菜单信息")
    @ApiImplicitParam(name = "role", value = "角色实体role", required = true, dataType = "RoleVo")
    @Log(value = "更新角色APP菜单",businessType = BusinessType.UPDATE)
    public ResponseBean<Boolean> roleAppMenuUpdate(@RequestBody Role role) {
        boolean success = false;
        String appMenuIds = role.getAppMenuIds();
        if (StringUtils.isNotBlank(role.getId())) {
            role = roleService.get(role);
            // 保存角色菜单关系
            if (role != null && StringUtils.isNotBlank(appMenuIds))
                success = appRoleMenuService.saveAppRoleMenus(role.getId(), Stream.of(appMenuIds.split(",")).collect(Collectors.toList())) > 0;
        }
        return new ResponseBean<>(success);
    }
    ...
    @ApiOperation(value = "验证角色CODE是否存在", notes = "验证角色CODE是否存在")
    @ApiImplicitParam(name = "id", value = "角色ID", required = true, paramType = "path")
    @Log(value = "验证角色CODE是否存在",businessType = BusinessType.INSERT)
    public ResponseBean<Boolean> checkRoleCode(@RequestParam("roleCode") String roleCode) {
        return new ResponseBean<>(roleService.checkRoleCode(roleCode) > 0);
    }
}
