...
1. `const basicAuthorization = 'Basic ' + btoa('web_app:as-microservice-as-secret')`
2. 密码需要加密，具体看**密码加密**部分
3. 请求头要带`Tenant-Code`，即租户标识
4. 请求头可以带`Project-Id`，即项目ID，如果为null，则获取全部项目
...
```
{
    ...
    'Tenant-Code': 'gitee',  // 租户标识，目前固定为gitee
     `Project-Id`:null //，即项目ID，如果为null，则获取全部项目
}
```
Authorization为`clientId:clientSecret`的base64编码，即：`'Basic ' + btoa('web_app:as-microservice-as-secret')`
...
export function refreshToken () {
  ...
  return request({
    url: '/api/auth/oauth/token',
    headers: {
      'Authorization': 'Basic ' + btoa('web_app:as-microservice-as-secret')
    },
    method: 'post',
