...
import com.aswl.iot.dto.Device;
...
@Component
public class CloseDCPowerTask implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        try {
            ...
            if (deviceArr.length > 0) {
                for (String deviceStr : deviceArr) {
                    String[] split = deviceStr.split("@");
                    ...
                    String[] channel = split[1].split(",");
                    for (String port : channel) {
                        command.setOperationCodeB(Integer.parseInt(port));
                        Thread.sleep(500);
