...
public class Client {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // String sendMsg="{\"eventNo\":1577676158530,\"data\":{\"deviceId\":\"660057792979275776\",\"operCode\":\"ACPowerOpen\",\"portSerial\":1}}";
        String sendMsg="{\"eventNo\":1577675446661,\"data\":{\"deviceId\":\"660057792979275776\",\"operCode\":\"PowerRestart\",\"portSerial\":0}}";
        // String sendMsg="{\"eventNo\":1577677143708,\"data\":{\"deviceId\":\"660057792979275776\",\"operCode\":\"ACPowerOpen\",\"portSerial\":1}}";
        // String sendMsg="{\"eventNo\":1577677251761,\"data\":{\"deviceId\":\"660057792979275776\",\"operCode\":\"ACPowerClose\",\"portSerial\":1}}";
        ...
        if (WebSocketConfig!=null) {
            ...
            log.info("成功使用配置文件加载客户端");
        }else {//默认参数连接到客户端
            myClient.runStompClient(myClient,
                    "ws://192.168.200.240:8097/as-push",
                    "/queue/troubleReport","/app/controlDevice",sendMsg);
            log.info("使用默认参数加载客户端");
        }

        // while (!myClient.RecvFlag) {
        //     //持续等待返回标志位为true
        //     log.info("-------------------持续等待返回验证消息中……，当前flag:"+myClient.RecvFlag);
        //     Thread.sleep(3000);
        // }
        //关闭所有连接终止程序
    ...
    public ListenableFuture<StompSession> connect(String url) {
        ...
        org.springframework.web.socket.sockjs.client.SockJsClient sockJsClient = new SockJsClient(transports);
        ...
        if (file.exists()) {
            ...
            try {
                ...
                while ((readline=reader.readLine())!=null) {
                    log.info("当前行内容："+readline);
                    String readStr []=readline.split("=");
                    if (readStr==null||readStr.length!=2) {
                        log.error("配置文件格式不符合规范，必须一行一个配置，并用‘=’分割，当前行内容："+readline);
                    }
                    ConfigMap.put(readStr[0], readStr[1]);
                }
                log.info("文件读取完成,最终的配置信息："+ConfigMap);
                boolean notice=false;
                for (int i = 0; i < keys.length; i++) {
                    if (!ConfigMap.containsKey(keys[i])) {
                        log.error("缺少对关键参数："+keys[i]+"的配置，配置将无法生效");
                        notice=true;
                    }
                }
                ConfigMap=notice? null:ConfigMap;
            } catch (Exception e) {
                log.info("文件读取过程发生异常："+e.getMessage());
            }finally{
                if (reader!=null) {
                    try {
                        FIS.close();
            ...
            log.info("不存在配置文件，请检查路径：");
            log.info("开始使用默认socketConfig");
        }
